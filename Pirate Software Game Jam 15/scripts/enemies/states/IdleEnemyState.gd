...
@export var idleDuration : float = 1
...
func _wait_for_player() -> void:
	if idleTimer.is_stopped() and global_position.distance_to(stateMachine.actor.player.global_position) < stateMachine.distanceToChase:
		idleTimer.stop()
